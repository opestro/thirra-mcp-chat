...
async function makeCloudflareRequest<T>(endpoint: string, options: RequestInit = {}): Promise<T | null> {
...
// Interface for Cloudflare RAG response
...
server.tool(
  "cloudflare_rag_search",
  "Search the Cloudflare RAG database for relevant information",
  {
  ...
  async ({ query, rag_name, limit = 10 }: { query: string; rag_name?: string; limit?: number }) => {
    ...
    const response = await makeCloudflareRequest<CloudflareRAGResponse>(endpoint, {
      method: "POST",
...
server.tool(
  "cloudflare_rag_info",
  "Get information about the configured Cloudflare RAG database",
  {
    rag_name: z.string().optional().describe("The name of the RAG database (defaults to configured RAG name)"),
  },
  async ({ rag_name }: { rag_name?: string }) => {
    const ragName = rag_name || CLOUDFLARE_RAG_NAME;
